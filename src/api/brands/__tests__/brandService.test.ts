...
describe("brandService", () => {
  ...
  const mockBrands: Brand[] = [
    {
      ...
      stores: ["15af2cdc-f352-11e8-80cd-02e611b48058", "15af31b3-f352-11e8-80cd-02e611b48058"],
    ...
    },
    {
      ...
      consolidated: 0,
      default_location_description_markdown: "",
      products: ["f5c72f41-972d-42b6-9ac5-51bad2afd01f", "57186a73-7857-4684-bf82-b2bc7b8a1040"],
  ...
  beforeEach(() => {
    ...
    brandServiceInstance = new BrandService(brandRepositoryInstance, cache);
  });
  ...
  describe("findAll", () => {
    ...
    it("should return cached brands if available", async () => {
      (cache.get as Mock).mockReturnValue(mockBrands);

      const response = await brandServiceInstance.findAll();

      expect(cache.get).toHaveBeenCalledWith("all_brands");
      expect(response.responseObject).toEqual(mockBrands);
    });

    it("should fetch brands from repository if not in cache", async () => {
      ...
      expect(cache.get).toHaveBeenCalledWith("all_brands");
      expect(brandRepositoryInstance.findAllAsync).toHaveBeenCalled();
      expect(cache.set).toHaveBeenCalledWith("all_brands", mockBrands);
      expect(response.responseObject).toEqual(mockBrands);
    });
